...
```{r}
Ex <- as.vector(matrix(jbc2$expdeath, ncol=Time, byrow=FALSE))
Yx <- as.vector(matrix(jbc2$death, ncol=Time, byrow=FALSE))
```
...
out %>%
    ...
    kableExtra::kbl(caption="Clustack Cell-Wise Confidence Bounds and Stacked Relative Risk Estimates") %>%
