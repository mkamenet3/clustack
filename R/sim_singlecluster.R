        ...
        for (rad in radii){
            for (risk in risk.ratios){
                ...
                tabn.loc <- rbind(cbind(IC="BIC",rad, risk, cent, theta,
                                        ...
                                        mod=model, pow=outpow.bic, fp = outfp.bic, type="NA",time=sim_superclust_loc.time, method="loc"),
                                  cbind(IC="AIC",rad, risk, cent, theta,
                                        time=as.numeric(paste(tim, collapse="")),
                                        mod=model, pow=outpow.aic, fp = outfp.aic, type="NA",time=sim_superclust_loc.time, method="loc"),
                                  cbind(IC="AICc",rad, risk, cent, theta,
                                        time=as.numeric(paste(tim, collapse="")),
                                        mod=model, pow=outpow.aicc, fp = outfp.aicc, type="NA",time=sim_superclust_loc.time, method="loc"))
                ...
                tabn.pc <- rbind(cbind(IC="BIC",rad, risk, cent, theta,
                                       time=as.numeric(paste(tim, collapse="")),
                                       mod=model, pow=outpow.bic, fp = outfp.bic, type="NA",time=sim_superclust_pc.time, method="pc"),
                                 cbind(IC="AIC",rad, risk, cent, theta,
                                       time=as.numeric(paste(tim, collapse="")),
                                       mod=model, pow=outpow.aic, fp = outfp.aic, type="NA",time=sim_superclust_pc.time, method="pc"),
                                 cbind(IC="AICc",rad, risk, cent, theta,
                                       time=as.numeric(paste(tim, collapse="")),
                                       mod=model, pow=outpow.aicc, fp = outfp.aicc, type="NA",time=sim_superclust_pc.time, method="pc"))
                ...
                #run clusso
                if(model=="space"){
                    sim_clusso.time <- system.time(sim_clusso <- lapply(1:nsim, function(i) clusso(df=jbcSIM[[i]],
                                                                                                   expected = expected,
                                                                                                   observed = observed,
                                                                                                   timeperiod = period,
                                                                                                   covars=FALSE,
                ...
                tabn.clusso <- rbind(cbind(IC="BIC",rad, risk, cent, theta,
                                           time=as.numeric(paste(tim, collapse="")),
                                           mod=model, pow=outpow.bic.qp, fp = outfp.bic.qp, type = "QP",time=sim_clusso.time, method="clusso"),
                                     cbind(IC="AIC",rad, risk, cent, theta,
                                           time=as.numeric(paste(tim, collapse="")),
                                           mod=model, pow=outpow.aic.qp, fp = outfp.aic.qp, type = "QP",time=sim_clusso.time, method="clusso"),
                                     cbind(IC="BIC",rad, risk, cent, theta,
                                           time=as.numeric(paste(tim, collapse="")),
                                           mod=model, pow=outpow.bic.p, fp = outfp.bic.p, type = "Pois",time=sim_clusso.time, method="clusso"),
                                     cbind(IC="AIC",rad, risk, cent, theta,
                                           time=as.numeric(paste(tim, collapse="")),
                                           mod=model, pow=outpow.aic.p, fp = outfp.aic.p, type = "Pois",time=sim_clusso.time, method="clusso"))
                ...
                tabn.fstagewise <- rbind(cbind(IC="BIC",rad, risk, cent, theta,
                                               time=as.numeric(paste(tim, collapse="")),
                                               mod=model, pow = outpow.stage.bic,fp = outfp.stage.bic, type="NA", time = sim_stage.time  ,method = "fstagewise" ),
                                         cbind(IC="AIC",rad, risk, cent, theta,
                                               time=as.numeric(paste(tim, collapse="")),
                                               mod=model, pow = outpow.stage.aic,fp = outfp.stage.aic, type="NA", time =  sim_stage.time ,method = "fstagewise" ))
