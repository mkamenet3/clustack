...
#####################################################################################
    ...
    out_bounds <- rbind(tabn.loc ,tabn.pc)
    if(model=="space"){
        out$stsmodel <- "space"
        out_bounds$stsmodel <- "space"
    }else{
        out$stsmodel <- "ST"
        out_bounds$stsmodel <- "ST"
    }
    write.csv(out, file=paste0(path.tables,"theta",theta,"_singlecluster.csv"), row.names = TRUE)    
    write.csv(out_bounds, file=paste0(path.tables,"theta",theta,"_singlecluster_bounds.csv"), row.names = TRUE)
