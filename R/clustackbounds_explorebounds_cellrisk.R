...
for(risk in risks){
    for(ecount in ecounts){
        for(rad in radii){
            ...
            clusterRR_ilarge <- lapply(1:nsim, function(i) sim_superclust_pc_large[[i]]$Lambda_dense) 
            ...
            outmataTsqrt.time <- system.time(outmataTsqrt <- lapply(1:nsim, function(i) matabounds(thetai=clusterRR_ilarge[[i]][,cellsix], 
                                                                                                   thetaa = cluster_thetaa[[i]][cellsix], 
                                                                                                   w_q=wslarge[[i]], 
                                                                                                   sparsematrix=t(sparseMAT), 
                                                                                                   outExp = Ex[[i]][cellsix],
            ...
            # a <- matrix(as.vector(nonma[[1]]),ncol=3)
            # b <- matrix(as.vector(nonma[[2]]),ncol=3)
            # 
            # aa <- sapply(1:nsim, function(x) matrix(as.vector(nonma[[x]]), ncol=3))
            ...
            master$outmataTsqrt.time <- rep(outmataTsqrt.time[[3]], nsim*length(cellsix))
            ...
            names(master) <- c("nonma.LB", "clusterMA", "nonma.UB",
                               ...
                               "outmataTsqrt.time","outmataTlog.time", "cellid")
