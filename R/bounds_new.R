...
calcbounds.cells <- function(id_ic, IC, res, byloc=FALSE, Ex, Yx,w, thetaa,thetai, sparsemat, cellsix, conf.level=0.95){
    ...
    if(id_ic==0){
        ...
        thetai <- selectuniqRR(t(aa)) #66870
...
#'@param thetai Unique individual cell relative risk estimates (unstacked).
#'@param var_thetai Variance for each unique individual cell relative risk estimate.
#'@param res Resultant object from \code{detectclusters()}.
...
#'@param transform If a transformation is used. Default is \code{"log"}. Other transformation TBD.
...
bucklandbounds.cells <- function(thetaa, thetai, var_thetai, res, w, sparsemat, IC, transform="log", overdisp.est, cellsix, conf.level){
